...
#if defined(NDEBUG)
# error "Afrolitecash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x30b1a703a11de34b70b946a0d9a86152be311dc0f726e55267339e4d1057f593");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Afrolitecash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Afrolitecash Signed Message:\n";
...
{
    // Afrolitecash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Afrolitecash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Afrolitecash: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Afrolitecash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Afrolitecash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Afrolitecash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3671f58c936dcb717cccc7ec9ba0c87bb711d2620271bd95ce359cb358f74797");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PSG vs OM game tonight at Paris Saint Germain";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a826c82539836d121c268bb83ad1c57aa7c6326012d9d7cbe7aa85c8e213abc11e4d4331db5f12b6422195d4de73eb27fed3d12bf1bbaa41e4cf42ac00353c0d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519579748;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085720362;

        if (fTestNet)
        {
            block.nTime    = 1519579702;
            block.nNonce   = 387533816;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd17292de086f5d5a774dbb76221d9fa85c0903091a0c08230565312c7bac03e0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb1, 0xdc }; // Afrolitecash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AfrolitecashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AfrolitecashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AfrolitecashMiner(CWallet *pwallet)
{
    printf("AfrolitecashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("afrolitecash-miner");
    ...
    try { loop {
        ...
        printf("Running AfrolitecashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AfrolitecashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AfrolitecashMiner, pwallet));
}
